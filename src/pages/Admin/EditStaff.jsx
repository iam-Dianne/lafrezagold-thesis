...
const EditStaff = () => {
  ...
  // Fetch the current staff details (simulate fetch for this example)
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `http://localhost/lafreza-server/staff/edit_staff.php?id=${id}`,
          {
            method: "GET",
        ...
        if (response.ok) {
          ...
          setErrorMessage(result.message || "Failed to load staff details");
          setLoading(false);
        }
      } catch (error) {
  ...
  const editName = async () => {
    const dataToSend = { ...editData, id: id }; // Add id here
    console.log("Sending data:", dataToSend); // Log the data being sent
    try {
      const response = await fetch(
        `http://localhost/lafreza-server/admin/edit_staff.php?staff_id=${id}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (result.success) {
        ...
        setStaff({ ...editData }); // Update staff details after successful edit
  ...
  return (
    <div className="flex justify-center mt-3">
      <div className="back-container">
        <Link to={"/staff/manage-staffs"} className="flex items-center">
      ...
      <div className="text-gray-900 mt-10 flex flex-col justify-between sm:w-[600px] xs:w-[400px]">
        <div className="content flex w-full justify-between">
          <div className="staff-fullname w-2/3">
